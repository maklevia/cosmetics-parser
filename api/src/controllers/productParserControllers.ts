import { InvalidLinkError, StoreRequestError } from "@api/parsers/errors/ParserErrors.js";
    ...
    parse: async (req: Request, res: Response) => {
        try {
            ...
            if (error instanceof InvalidLinkError) {
                ...
                res.status(400).json({error: 'Cannot find your product. Please, check if your link is correct, or try again later.'})
            } else  {
    ...
    addProduct: async (req: Request, res: Response) => {
        try {
            ...
            if (error instanceof DuplicateProductError) {
                res.status(409).json({error: 'This product is already in your Collection.'})
            } else {
